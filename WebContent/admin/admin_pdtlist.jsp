...
String tmp = "&ac="+ac+"&ap="+ap+
	"&orderby="+orderby+"&key="+key+"&schtype="+schtype; //쿼리스트링
...
<div class="wrapper">
	<div class="menu">
		...
		%>
			<a href = "product_list?<%=tmp%>"><%=small.getAp_name() %></a>
		<%} %>
	</div>
	...
	<form name = "schForm">
		...
		<select name = "schtype">
			...
			<option value = "c" <%if(schtype!=null && schtype.equals("p")){%> selected = "selected" <%} %>>상품분류</option>
		</select>
	...
	<%
	char isview = pi.getPi_isview().charAt(0);
	if(isview=='n'){
		out.print("<p class = 'isview'>미게시</p>");
	}
	...
	%>
		<figure>
			<img src="product/product_img/<%=pi.getPi_img1()%>">
		</figure>
		<div>
			<p class = "info">
			...
			<p class = "manage">
				<a href="product_form_up?piid=<%=pi.getPi_id() %>" class = "sbutton">수정</a>
				<a href="product_proc_del?piid=<%=pi.getPi_id() %>" class = "sbutton">삭제</a>
			</p>
		</div> 
	</li>
...
<% }%>
	...
	if(cpage!=1){
		out.println("<a href='product_list?cpage=1"+tmp+"'>처음</a>");
	}else{ 
		out.println("<p>처음</p>");
	}
	
	if(cpage>1){
		out.println("<a href='product_list?cpage="+(cpage-1)+tmp+"'>이전</a>");
	}else{ 
		out.println("<p>이전</p>");
	}
	...
	for(int i = 1; i<=pcnt;i++){
		if(cpage == i){
			...
			out.println("<a href = 'product_list?cpage="+i+tmp+"'>"+i+"</a>");
		}
	}
	
	if(cpage<pcnt){
		out.println("<a href='product_list?cpage="+(cpage+1)+tmp+"'>다음</a>");
	}else{ 
		out.println("<p>다음</p>");
	}
	
	if(cpage!=pcnt){
		out.println("<a href='product_list?cpage="+pcnt+tmp+"'>마지막</a>");
	}else{ 
		out.println("<p>마지막</p>");
	}
	%>
...
<% }%>
	<div class = "btn">
	<a href = "product_form_in" class = "button">등록</a>
	</div>
</div>
